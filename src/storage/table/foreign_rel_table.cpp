...
ForeignRelTableScanState::ForeignRelTableScanState(MemoryManager& mm, common::ValueVector* nodeIDVector,
    ...
    for (size_t i = 0; i < this->outputVectors.size(); ++i) {
        dataChunk.valueVectors[i] = std::shared_ptr<common::ValueVector>(this->outputVectors[i], [](common::ValueVector*){});
    }
...
void ForeignRelTable::initScanState(transaction::Transaction* transaction, TableScanState& scanState,
    ...
    function::TableFuncInitLocalStateInput localInput{*foreignRelScanState.sharedState, *scanBindData, nullptr /* clientContext */};
    foreignRelScanState.localState = scanFunction.initLocalStateFunc(localInput);
}

bool ForeignRelTable::scanInternal(transaction::Transaction* transaction, TableScanState& scanState) {
    auto& foreignRelScanState = static_cast<ForeignRelTableScanState&>(scanState);
    function::TableFuncInput input{scanBindData.get(), foreignRelScanState.localState.get(), foreignRelScanState.sharedState.get(), nullptr /* clientContext */};
    common::DataChunk dc;
...
common::row_idx_t ForeignRelTable::getNumTotalRows(const transaction::Transaction* transaction) {
